...
public class PingPongIT {
    ...
    public void container_should_have_static_ip_for_app_net_network() throws InterruptedException, IOException {
        ...
        CubeOutput cubeOutput = dockerClient.execStartCmd(execCreateCmdResponse.getId()).withDetach(false)
            .exec(new DockerOutputResultCallback()).awaitCubeOutput();

        assertThat(cubeOutput.getStandard()).contains("inet addr:172.16.238.10",
    ...
    public void network_should_reach_pingpong_by_alias_ping() throws UnsupportedEncodingException, InterruptedException {
        ...
        DockerOutputResultCallback callback = new DockerOutputResultCallback();
        dockerClient.execStartCmd(exec.getId()).exec(callback).awaitCompletion(2, TimeUnit.SECONDS);
        assertThat(callback.getCubeOutput().getError()).isEqualToIgnoringWhitespace(EXPECTED_RESPONSE);
    }
    ...
    public void network_should_reach_pingpong_by_alias_foo() throws UnsupportedEncodingException, InterruptedException {
        ...
        DockerOutputResultCallback callback = new DockerOutputResultCallback();
        dockerClient.execStartCmd(exec.getId()).exec(callback).awaitCompletion(2, TimeUnit.SECONDS);
        assertThat(callback.getCubeOutput().getStandard()).isEqualToIgnoringWhitespace(EXPECTED_RESPONSE);
    }
}
